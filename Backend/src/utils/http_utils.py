...
def send_json_response(handler, status_code, data):
    ...
    handler.send_header('Access-Control-Allow-Origin', '*') # CORS middlewrae
    handler.end_headers()
    handler.wfile.write(json.dumps(data, cls=CustomJSONEncoder).encode('utf-8'))

def parse_json_body(handler):
    #Função para ler a response e serializar (transformar em json)
    try:
...
def get_auth_user(handler):
    #Retorna os dados do usuário através do header com o jwt
